...
class OntologyProc(object):

	def __init__(self, ontology_path, hiearchies_path):
		"""
		Load ontology and set use-case variable 

		Params:
			ontology_path (str): ontology.owl fpath
			hierarchies_path (str): hierarchical relations fpath

		Returns: None
		"""

		self.ontology = owlready2.get_ontology(ontology_path).load()
		self.disease = {'colon': 'colon carcinoma', 'lung': 'lung cancer', 'cervix': 'cervical cancer', 'celiac': 'celiac disease'}
		self.hrels = utils.read_hierarchies(hiearchies_path)
	...
	def restrict2use_case(self, use_case, limit=1000):
		...
		sparql = "PREFIX exa: <https://w3id.org/examode/ontology/> \
				  ...
				  select ?iri ?iri_label ?iri_SNOMED_code ?iri_UMLS_code ?semantic_area ?semantic_area_label where { \
            		  ...
            		  OPTIONAL {?iri exa:hasUMLS ?iri_UMLS_code .} \
		...
		for e in r:
			...
			ontology_dict['semantic_area_label'].append(e[5].toPython() if e[5] else None)			
		return pd.DataFrame(ontology_dict)	

	def lookup_semantic_areas(self, semantic_areas, use_case_ontology):
		"""
		...
		if type(semantic_areas) == list:  # search for list of semantic areas
			...
			rows = use_case_ontology.loc[use_case_ontology['semantic_area_label'] == semantic_areas][['iri', 'label', 'semantic_area_label']]
	...
	def get_ancestors(self, concepts, include_self=False):
		...
		if type(concept) == owlready2.entity.ThingClass:  # class level - rely on ancestors() property (recursively return all parent classes)
	...
	def merge_nlp_and_struct(self, nlp_concepts, struct_concepts):
		...
		for sem_area in nlp_concepts.keys():
			if nlp_concepts[sem_area] and struct_concepts[sem_area]:  # semantic area is not empty for both 'nlp' and 'struct' sections
				...
				IRIs = {self.get_higher_concept(combination[0][0], combination[1][0]) for combination in combinations} - {None}
				# remove under-specified concepts and store remaining concepts
				cconcepts[sem_area] = deepcopy(nlp_concepts[sem_area])
				cconcepts[sem_area].extend([concept for concept in struct_concepts[sem_area] if concept[0] not in [concept[0] for concept in nlp_concepts[sem_area]]])
	...
	def aoec_merge_concepts(self, concepts):  # @smarchesin TODO: change name from aoec_merge_concepts to merge_concepts - applies also to Radboud
		"""
		Merge the information extracted from nlp and structured sections
		
		Params:
			concepts (dict): the dictionary of linked concepts
		
		Returns: a dict containing the linked concepts for each report w/o distinction between 'nlp' and 'struct' concepts
		"""
		
		cconcepts = dict()
		# loop over AOEC reports and merge linked concepts
		for rid, rconcepts in concepts.items():
			cconcepts[rid] = dict()
			# merge linked concepts from 'nlp' and 'struct' sections
			for sem_area in rconcepts['nlp'].keys():
				if rconcepts['nlp'][sem_area] and rconcepts['struct'][sem_area]:  # semantic area is not empty for both 'nlp' and 'struct' sections
					# get all the possible combinations of 'nlp' and 'struct' concepts
					combinations = list(itertools.product(rconcepts['nlp'][sem_area], rconcepts['struct'][sem_area]))
					# return IRIs to be removed (hierarchically higher)
					IRIs = {self.get_higher_concept(combination[0][0], combination[1][0]) for combination in combinations} - {None}
					# remove under-specified concepts and store remaining concepts
					cconcepts[rid][sem_area] = deepcopy(rconcepts['nlp'][sem_area])
					cconcepts[rid][sem_area].extend([concept for concept in rconcepts['struct'][sem_area] if concept[0] not in [concept[0] for concept in rconcepts['nlp'][sem_area]]])
					# remove IRIs from cconcepts
					cconcepts[rid][sem_area] = [concept for concept in cconcepts[rid][sem_area] if concept[0] not in IRIs]
				elif rconcepts['nlp'][sem_area]:  # semantic area is not empty only for the 'nlp' section
					cconcepts[rid][sem_area] = deepcopy(rconcepts['nlp'][sem_area])
				elif rconcepts['struct'][sem_area]:  # semantic area is not empty only for 'struct' section
					cconcepts[rid][sem_area] = deepcopy(rconcepts['struct'][sem_area])
				else:  # semantic area is empty for both sections
					cconcepts[rid][sem_area] = list()
		# return combined concepts
		return cconcepts
	...
	def merge_diagnosis_and_conclusions(self, diagnosis, conclusions):  # @smarchesin TODO: deprecated? 
		"""
		Merge the concepts extracted from the diagnosis and conclusions sections by keeping the most specific concepts and removing duplicates
		
		Params:
			diagnosis (dict): the diagnosis dict of linked concepts
			conclusions (dict): the conclusions dict of linked concepts
			
		Returns: a dict containing the linked concepts w/o distinction between 'diagnosis' and 'conclusions' concepts
		"""
		
		cconcepts = dict()
		# merge linked concepts from 'diagnosis' and 'conclusions' sections
		for sem_area in diagnosis.keys():
			if diagnosis[sem_area] and conclusions[sem_area]:  # semantic area is not empty for both 'diagnosis' and 'conclusions' sections
				# get all the possible combinations of 'nlp' and 'struct' concepts
				combinations = list(itertools.product(diagnosis[sem_area], conclusions[sem_area]))
				# return IRIs to be removed (hierarchically higher)
				IRIs = {self.get_higher_concept(combination[0][0], combination[1][0]) for combination in combinations} - {None}
				# remove duplicate and under-specified concepts and store remaining concepts
				cconcepts[sem_area] = deepcopy(diagnosis[sem_area])
				cconcepts[sem_area].extend([concept for concept in conclusions[sem_area] if concept[0] not in [concept[0] for concept in diagnosis[sem_area]]])
				# remove IRIs from cconcepts
				cconcepts[sem_area] = [concept for concept in cconcepts[sem_area] if concept[0] not in IRIs]
			elif diagnosis[sem_area]:  # semantic area is not empty only for the 'diagnosis' section
				cconcepts[sem_area] = deepcopy(diagnosis[sem_area])
			elif conclusions[sem_area]:  # semantic area is not empty only for 'conclusions' section
				cconcepts[sem_area] = deepcopy(conclusions[sem_area])
			else:  # semantic area is empty for both sections
				cconcepts[sem_area] = list()
		# return combined concepts
		return cconcepts